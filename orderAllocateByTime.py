    ...
    for i in range(N):
        ...
        for j in range(M):
            if k[j] == 1:  # 是否是第一个
                ...
                FinishTime[j, k[j] - 1] = FinishTime[j, k[j] - 2] + pt[chro1[i], int(Mindex[j, k[j] - 2])] + wt[chro1[i], j]  # 上一个完成时间+当前订单在前一个订单机器上的准备时间+当前订单加工时间
                Tc[chro1[i], j] = FinishTime[j, k[j] - 1]
        ...
        print('SelectionProbabilityTemp2', SelectionProbabilityTemp2)
        # 获取从小到大的索引值
